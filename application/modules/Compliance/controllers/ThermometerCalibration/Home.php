...
class Home extends MY_Controller {
    ...
    {   
        ...
        $data['site_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibSites', array(), ['status=1', 'location_id' => $this->selected_location_id, 'is_deleted' => 0]); 
        $data['prep_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibPrepArea', array(), ['status=1', 'location_id' => $this->selected_location_id, 'is_deleted' => 0]); 
        
        // Fetch active products (equipment)
        $condition = array('status' => 1, 'is_deleted' => 0, 'location_id' => $this->selected_location_id); 
        $data['products'] = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibProducts', '', $condition);
        
        // Today's entered data
        $condition = array('date_entered' => date('Y-m-d'), 'location_id' => $this->selected_location_id);
        $history_data = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalib_history', '', $condition);

        // Process history data into $todaysEnteredData format keyed by product_id
        ...
        $this->load->view('ThermometerCalibration/dashboard', $data);
    ...
    public function saveDashboardData()
    {
        ...
        $exists = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalib_history', '', $conditionPr);

        if ($exists) {
            $this->common_model->commonRecordUpdate('Compliance_ThermometerCalib_history', 'id', $exists[0]['id'], $data);
        } else {
            $this->common_model->commonRecordCreate('Compliance_ThermometerCalib_history', $data);
        }
    ...
    function history()
    {
        $data['site_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibSites', '', ['status' => 1, 'location_id' => $this->selected_location_id, 'is_deleted' => 0]);
        
        $this->load->view('general/header');
        $this->load->view('ThermometerCalibration/history', $data);
        $this->load->view('general/footer');
    }
    ...
    public function historyData($encodedDateRange = '', $site_id = '') 
    {
        ...
        if (count($dateParts) == 2) {
            ...
            // Fetch data
            $data['site_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibSites', '', array('location_id' => $this->selected_location_id, 'is_deleted' => 0)); 
            $data['prep_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibPrepArea', '', array('location_id' => $this->selected_location_id, 'is_deleted' => 0)); 
            $condition = array('status' => 1, 'is_deleted' => 0, 'location_id' => $this->selected_location_id);
            $data['products'] = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibProducts', '', $condition);
            ...
            if ($site_id != '' && $site_id != 'Select Site') {
                $prepAreas = $this->common_model->fetchRecordsDynamically(
                    'Compliance_ThermometerCalibPrepArea', 
                    ['id'], 
            ...
            $history_data = $this->tenantDb->get('Compliance_ThermometerCalib_history')->result_array();
    ...
    public function updateHistory() 
    {
        ...
        if ($check_date !== '' && $check_date !== NULL) {
            $update_data['check_date'] = $check_date;
        }
        ...
        $exists = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalib_history', '', $condition);

        if (!empty($exists)) {
            // Update existing record
            $this->common_model->commonRecordUpdateMultipleConditions('Compliance_ThermometerCalib_history', $condition, $update_data);
            ...
            // Insert new record
            $insert_id = $this->common_model->commonRecordCreate('Compliance_ThermometerCalib_history', $data);
            if ($insert_id) {
    ...
    function listProduct()
    {
        $condition = array('status' => 1, 'is_deleted' => 0, 'location_id' => $this->selected_location_id);
        $data['products'] = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibProducts', '', $condition);
        $data['site_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibSites', '', $condition);
        $data['prep_detail'] = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibPrepArea', '', $condition);
        
        $this->load->view('general/header');
        $this->load->view('ThermometerCalibration/listProduct', $data);
        $this->load->view('general/footer');
    }
    ...
    public function addOrUpdateProduct() 
    {
        ...
        if ($id) {
            ...
            $this->common_model->commonRecordUpdate('Compliance_ThermometerCalibProducts', 'id', $id, $data);
        } else {
            $data['created_at'] = date('Y-m-d');
            $this->common_model->commonRecordCreate('Compliance_ThermometerCalibProducts', $data);
        }

        redirect('Compliance/ThermometerCalibration/Home/listProduct');
    }
    
    public function getProductById($id) 
    {
        $condition = array('id' => $id);
        $product = $this->common_model->fetchRecordsDynamically('Compliance_ThermometerCalibProducts', '', $condition);
    ...
    public function updateSortOrder()
    {
        ...
        foreach ($newOrder as $index => $itemId) {
            ...
            $this->tenantDb->update('Compliance_ThermometerCalibProducts');
        }
        echo "success";
    }
}
