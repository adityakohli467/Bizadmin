                ...
                <div class="card-header border-0">
                    <div class="d-flex align-items-center gap-2">
                        <h5 class="card-title mb-0 flex-grow-1 text-dark">
                            <i class="fa-solid fa-utensils"></i> Kitchen Production Products
                        </h5>
                        <a href="<?php echo base_url('Compliance/KitchenProduction/Home') ?>" class="btn btn-secondary btn-sm">
...
<div class="modal fade" id="productModal" tabindex="-1" aria-labelledby="productModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form id="productForm" action="<?php echo base_url('Compliance/KitchenProduction/Home/addOrUpdateProduct'); ?>" method="post">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="productModalLabel"><i class="fa-solid fa-utensils"></i> Add/Edit Product</h5>
...
$(document).ready(function() {
    ...
    $("#sortable").sortable({
        ...
        update: function(event, ui) {
            ...
            $.ajax({
                url: "<?php echo base_url('Compliance/KitchenProduction/Home/updateSortOrder'); ?>",
                type: "POST",
...
function openProductModal() {
    $('#productForm')[0].reset();
    $('#product_id').val('');
    $('#productModalLabel').html('<i class="fa-solid fa-utensils"></i> Add Product');
}

function editProduct(id) {
    $.get("<?= base_url('Compliance/KitchenProduction/Home/getProductById/') ?>" + id, function(res) {
...
$(document).on("click", "#delete-record", function() {
    if (deleteProductId) {
        $.ajax({
            type: "POST",
            url: "<?= base_url('Compliance/KitchenProduction/Home/delete') ?>",
            data: { id: deleteProductId, table_name: 'Compliance_KitchenProductionproducts' },
            success: function(response) {
                if (response == 'success') {
