            ...
            <div class="card py-3" id="orderList">
                <div class="card-header border-0">
                    <div class="d-flex align-items-center gap-2">
                        <h5 class="card-title mb-0 flex-grow-1 text-dark">
                            <i class="fa-solid fa-thermometer-half"></i> Thermometer Calibration Equipment
                        </h5>
                        <a href="<?php echo base_url('Compliance/ThermometerCalibration/Home') ?>" class="btn btn-secondary btn-sm">
                        ...
                        <table class="table table-nowrap align-middle" id="activeRow">
                            <thead class="table-light">
                                <tr class="text-uppercase text-muted">
                                    ...
                                    <th class="sort" data-sort="product_name">Equipment Name</th>
                            ...
                            <tbody class="list form-check-all" id="sortable">
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="5" class="text-center">No equipment found. <a href="#" data-bs-toggle="modal" data-bs-target="#productModal" onclick="openProductModal()">Add your first equipment</a>.</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                            ...
                            <div class="modal-content">
                                <div class="modal-body p-5 text-center">
                                    ...
                                    <div class="mt-4 text-center">
                                        <h4 class="text-dark">Are you sure you want to delete this equipment?</h4>
                                        <p class="fs-15 mb-4 text-muted">Deleting this equipment will remove all its information from the database.</p>
...
<div class="modal fade" id="productModal" tabindex="-1" aria-labelledby="productModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form id="productForm" action="<?php echo base_url('Compliance/ThermometerCalibration/Home/addOrUpdateProduct'); ?>" method="post">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="productModalLabel"><i class="fa-solid fa-thermometer-half"></i> Add/Edit Equipment</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="id" id="product_id">
                    <div class="mb-3">
                        <label for="product_name" class="form-label">Equipment Name <span class="text-danger">*</span></label>
                        <input type="text" name="product_name" class="form-control" id="product_name" placeholder="e.g., Asian Section Chef" required>
...
$(document).ready(function() {
    ...
    $("#sortable").sortable({
        ...
        update: function(event, ui) {
            ...
            $.ajax({
                url: "<?php echo base_url('Compliance/ThermometerCalibration/Home/updateSortOrder'); ?>",
                type: "POST",
...
function openProductModal() {
    $('#productForm')[0].reset();
    $('#product_id').val('');
    $('#productModalLabel').html('<i class="fa-solid fa-thermometer-half"></i> Add Equipment');
}

function editProduct(id) {
    $.get("<?= base_url('Compliance/ThermometerCalibration/Home/getProductById/') ?>" + id, function(res) {
        const data = JSON.parse(res)[0];
        $('#product_id').val(data.id);
        $('#product_name').val(data.product_name);
        $('#prep_id').val(data.prep_id);
        $('#productModalLabel').html('<i class="fa-solid fa-thermometer-half"></i> Edit Equipment');
...
$(document).on("click", "#delete-record", function() {
    if (deleteProductId) {
        $.ajax({
            type: "POST",
            url: "<?= base_url('Compliance/ThermometerCalibration/Home/delete') ?>",
            data: { id: deleteProductId, table_name: 'Compliance_ThermometerCalibProducts' },
            success: function(response) {
                if (response == 'success') {
                    $('#row_' + deleteProductId).fadeOut();
